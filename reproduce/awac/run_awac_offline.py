...
logger = CompositeLogger(log_path=f"./log/awac/offline/{args.name}", name=exp_name, loggers_config={
...
for i_epoch in trange(1, args.max_epoch+1):
    ...
    if i_epoch % args.save_interval == 0:
        logger.log_object(name=f"policy_{i_epoch}.pt", object=policy.state_dict(), path=f"./out/awac/offline/{args.name}/{args.task}/seed{args.seed}/policy/")
