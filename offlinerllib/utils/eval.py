...
def eval_decision_transformer(
    ...
    def eval_one_return(target_return, score_func=None):
        ...
        for _ in range(n_episode):
            states = np.zeros(
                [1, actor.episode_len + 1, actor.state_dim], dtype=np.float32
            )
            actions = np.zeros(
                [1, actor.episode_len + 1, actor.action_dim], dtype=np.float32
            )
            ...
            for step in range(actor.episode_len):
                ...
                returns_to_go[:, step + 1] = (
                    returns_to_go[:, step] - reward * return_scale
                )
        ...
        episode_lengths = np.asarray(episode_lengths)
        return {
            "normalized_score_mean_target{:.1f}".format(
                target_return
            ): episode_returns.mean(),
            "normalized_score_std_target{:.1f}".format(
                target_return
            ): episode_returns.std(),
...
@torch.no_grad()
def eval_online_policy(env: gym.Env, actor: nn.Module, n_episodes: int, seed: int):
    env.seed(seed)
    actor.eval()
    # actor.to(device)
    episode_lengths = []
...
@torch.no_grad()
def eval_online_policy_for_(env: gym.Env, actor: nn.Module, n_episodes: int, seed: int):
    env.seed(seed)
    actor.eval()
    # actor.to(device)
    episode_lengths = []
