...
def generate_preference_data(data, num_samples, segment_len, discount=0.99):
    ...
    for _ in range(num_samples):
        ...
        while trajectory_lengths[ep_idx1] < segment_len or trajectory_lengths[ep_idx2] < segment_len:
            ep_idx1 = random.randint(0, data['obs'].shape[0] - 1)
            ep_idx2 = random.randint(0, data['obs'].shape[0] - 1)
...
def generate_data(raw_data_path, prefix=""):
    ...
    saved_path = f"./datasets/rpl/{args.name}/{args.env}-{args.shift_level}/" if args.data_saved_path is None else args.data_saved_path + f"/{args.env}-{args.shift_level}/"
...
# offline data
generate_data(f"./datasets/rpl/{args.name}/{args.env}-{args.shift_level}/data.npz")

# replay data
generate_data(f"./datasets/rpl/{args.name}/{args.env}-{args.shift_level}/labeled_replay.npz", prefix="replay_")
